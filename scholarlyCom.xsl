...
<xsl:strip-space elements="*" />
  ...
  <xsl:template match="/">
    <p class="specifyResults">&#8594; my activities in <span class="tagDomain"> scholarly communication</span>

    <xsl:for-each select="activities/item">

      <xsl:if test="domain= 'scholarlyCom' ">
        <br /><br />
        <b><xsl:value-of select="date" /> </b>&#160;
        <span class="tagAction"><xsl:value-of select="action"/></span> &#160;<br />
        <!--<span class="tag"><xsl:value-of select="domain"/></span>  -->
